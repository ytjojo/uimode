 ...
 */
class FactoryMerger(private val before:ArrayList<Factory2> = ArrayList(), private val mAfter: UiModeInflaterFactory) : Factory2 {
    override fun onCreateView(name: String, context: Context, attrs: AttributeSet): View? {
        if (before.isNotEmpty()) {
            for (factory in before) {
                val v = factory.onCreateView(name, context, attrs)
                if (v != null){
                    UiModeDelegate.onViewCreated(v, attrs)
                    return v
                }
            }
        }
